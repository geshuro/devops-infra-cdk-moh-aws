...
describe('GET /api/user - Retrieve current user', () => {
  let setup;
  let adminSession;
  ...
  `('Negative Tests', ({ description, sessionType, spoofedJwtSubFn, code }) => {
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(session.resources.currentUser.get()).rejects.toMatchObject({ code });
    });
  });
  ...
  describe('Positive Tests', () => {
    it('should return current user information if user status is active', async () => {
      ...
      await expect(session.resources.currentUser.get()).resolves.toMatchObject({
        enabled: true,

...
class DbUpdater {
  constructor(log = console, client) {
    ...
    this.marked = {};
    this.createdAtState = { enabled: true, processed: false, value: 0 };
    this.updatedAtState = { enabled: true, processed: false, value: 0 };
  ...
  createdAt(timestamp) {
    ...
    if (!_.isNumber(timestamp) || _.isEmpty(_.trim(timestamp)))
      throw new Error(`DbUpdater.createdAt("${timestamp}" <== must be a number and can not be empty).`);
    this.createdAtState.enabled = true;
    this.createdAtState.value = timestamp;
    return this;
  }
  ...
  updatedAt(str) {
    ...
    if (!_.isNumber(str) || _.isEmpty(_.trim(str)))
      throw new Error(`DbUpdater.updatedAt("${str}" <== must be a number and can not be empty).`);
  ...
  item(item) {
    ...
    if (this.createdAtState.enabled && !this.createdAtState.processed) {
      ...
      createdAt = _.isEmpty(createdAt) ? Date.now() : createdAt;
    ...
    if (this.updatedAtState.enabled && !this.updatedAtState.processed) {
      ...
      updatedAt = _.isEmpty(updatedAt) ? Date.now() : updatedAt;

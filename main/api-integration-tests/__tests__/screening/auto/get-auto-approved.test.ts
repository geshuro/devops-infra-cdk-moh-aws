...
describe('GET /api/screening/:screeningId/autoApproved - Get auto approved documents', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    it('should return auto approved articles for admin user', async () =>
      expect(
        adminSession.resources.screenings.screening(adminScreening.id).autoApproved.getTotalItems(),
      ).resolves.toBeGreaterThan(0));

    it('should return auto approved articles for regular user', async () =>
      expect(
        userSession.resources.screenings.screening(userScreening.id).autoApproved.getTotalItems(),
      ).resolves.toBeGreaterThan(0));
  });
  ...
  const negativeTestFn = ({ description, sessionType, operationAttrs, spoofedJwtSubFn, code }) => {
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings.screening(adminScreening.id).autoApproved.getWithRetry(operationAttrs),

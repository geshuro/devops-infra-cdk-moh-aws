...
describe('GET /api/screening/:screeningId/autoRejected - Get auto rejected documents', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    it('should return auto rejected articles for admin user', async () =>
      expect(
        adminSession.resources.screenings.screening(adminScreening.id).autoRejected.getTotalItems(),
      ).resolves.toBeGreaterThan(0));

    it('should return auto rejected articles for regular user', async () =>
      expect(
        userSession.resources.screenings.screening(userScreening.id).autoRejected.getTotalItems(),
      ).resolves.toBeGreaterThan(0));
  });
  ...
  const negativeTestFn = ({ description, sessionType, operationAttrs, spoofedJwtSubFn, code }) => {
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings.screening(adminScreening.id).autoRejected.getWithRetry(operationAttrs),

...
describe('GET /api/screening/:screeningId/decision/bulk/approve - Get bulk document approval', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    ...
    `('approval', ({ description, getScreeningId, getSession }) => {
      it(description, async () => {
        ...
        await session.resources.screenings.screening(screeningId).bulkDecision('approve', query).get();
        ...
        expect(isEveryDecision(autoApprovedResponse.items, 'approve', 'manualDecisions')).toBe(true);
      });
    });
  });
    ...
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings
          ...
          .bulkDecision('approve', { autoDecision: autoDecision || 'rejected' })
          .get(),

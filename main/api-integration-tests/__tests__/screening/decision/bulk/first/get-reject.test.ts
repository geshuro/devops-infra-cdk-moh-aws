...
describe('GET /api/screening/:screeningId/decision/bulk/reject - Get bulk document reject', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    describe.each`
      ...
      ${'should return all articles as rejected for admin user'}   | ${() => adminScreening.id} | ${() => adminSession}
      ${'should return all articles as rejected for regular user'} | ${() => userScreening.id}  | ${() => userSession}
    `('approval', ({ description, getScreeningId, getSession }) => {
      it(description, async () => {
        ...
        await session.resources.screenings.screening(screeningId).bulkDecision('reject', query).get();
        ...
        expect(isEveryDecision(autoApprovedResponse.items, 'reject', 'manualDecisions')).toBe(true);
      });
    });
  });
    ...
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings
          ...
          .bulkDecision('reject', { autoDecision: autoDecision || 'rejected' })
          .get(),

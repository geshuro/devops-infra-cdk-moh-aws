...
describe('GET /api/screening/:screeningId/decision/bulk/reset - Get bulk document reset', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  beforeAll(async () => {
    ...
    await createArticlesWithDifferentDecisions(setup, adminSession, adminScreening.id);
    await createArticlesWithDifferentDecisions(setup, userSession, userScreening.id);
  });
  ...
  describe('Positive Tests', () => {
    describe.each`
      ...
      ${'should return all articles as indecisive for admin user'}   | ${() => adminScreening.id} | ${() => adminSession}
      ${'should return all articles as indecisive for regular user'} | ${() => userScreening.id}  | ${() => userSession}
    `('approval', ({ description, getScreeningId, getSession }) => {
      it(description, async () => {
        const screeningId = getScreeningId();
        const session = getSession();

        // reset all articles on the page determined by a query.
        ...
        await session.resources.screenings.screening(screeningId).bulkDecision('reset', query).get();
        ...
        const autoApprovedResponse = await session.resources.screenings
          ...
          .autoApproved.getWithRetry();
        expect(isEveryDecision(autoApprovedResponse.items, undefined, 'manualDecisions')).toBe(true);
      });
    });
  });
    ...
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings
          ...
          .bulkDecision('reset', { autoDecision: autoDecision || 'rejected' })
          .get(),

...
describe('GET /api/screening/:screeningId/second/decision/bulk/reject - Get bulk document rejection', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    ...
    `('approval', ({ description, getScreeningId, getSession }) => {
      it(description, async () => {
        ...
        await session.resources.screenings.screening(screeningId).secondBulkDecision('reject', query).get();
        ...
        expect(isEveryDecision(autoApprovedResponse.items, 'reject', 'secondManualDecisions')).toBe(true);
      });
    });
  });
    ...
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings
          ...
          .secondBulkDecision('reject', { autoDecision: autoDecision || 'rejected' })
          .get(),

...
describe('GET /api/screening/:screeningId/second/decision/bulk/reset - Get bulk document reset', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    ...
    `('approval', ({ description, getScreeningId, getSession }) => {
      it(description, async () => {
        ...
        await session.resources.screenings.screening(screeningId).secondBulkDecision('reset', query).get();
        ...
        expect(isEveryDecision(autoApprovedResponse.items, undefined, 'secondManualDecisions')).toBe(true);
      });
    });
  });
    ...
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings
          ...
          .secondBulkDecision('reset', { autoDecision: autoDecision || 'rejected' })
          .get(),

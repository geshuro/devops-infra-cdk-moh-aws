...
describe('GET /api/screening/:screeningId/decision/reject/:documentId - Get document rejection', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    ...
    `('approval', ({ description, getScreeningId, getArticleId, getSession }) => {
      it(description, async () => {
        ...
        // approve an article
        await session.resources.screenings.screening(screeningId).decision('reject', articleId).get();
    ...
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings
          ...
          .decision('reject', articleId || adminAutoApprovedArticle.id)
          .get(),

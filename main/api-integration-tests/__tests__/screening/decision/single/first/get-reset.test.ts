...
describe('GET /api/screening/:screeningId/decision/reset/:documentId - Get document reset', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    const resetDecisions = { manualDecisions: [] };
    describe.each`
    ...
    `('approval', ({ description, getScreeningId, getArticleId, getSession }) => {
      it(description, async () => {
        ...
        // reset the decision
        session.resources.screenings.screening(screeningId).decision('reset', articleId).get();
    ...
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings
          ...
          .decision('reset', articleId || adminAutoApprovedArticle.id)
          .get(),

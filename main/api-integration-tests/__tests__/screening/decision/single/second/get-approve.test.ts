...
describe('GET /api/screening/:screeningId/second/decision/approve/:documentId - Get document approval', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
    ...
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings
          ...
          .secondDecision('approve', articleId || adminAutoApprovedArticle.id)
          .get(),

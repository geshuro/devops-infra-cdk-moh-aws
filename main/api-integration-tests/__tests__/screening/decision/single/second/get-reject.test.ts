...
describe('GET /api/screening/:screeningId/second/decision/reject/:documentId - Get document rejection', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    ...
    describe.each`
      ...
      ${'should return article rejection for admin user'}   | ${() => adminScreening.id} | ${() => adminAutoApprovedArticle.id} | ${() => adminSession}
      ${'should return article rejection for regular user'} | ${() => userScreening.id}  | ${() => userAutoApprovedArticle.id}  | ${() => userSession}
    `('approval', ({ description, getScreeningId, getArticleId, getSession }) => {
      it(description, async () => {
        ...
        // approve an article
        await session.resources.screenings.screening(screeningId).secondDecision('reject', articleId).get();

        // TODO: OS Delay encapsulate
        // give some time for newly created articles to be streamed into OpenSearch
        await sleep(5000);

        await expect(getArticle(session, screeningId, articleId)).resolves.toMatchObject(rejectDecisions);
      });
    });
  });
    ...
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings
          ...
          .secondDecision('reject', articleId || adminAutoApprovedArticle.id)
          .get(),

...
describe('GET /api/screening/:screeningId/summary1 - Get screening summary 1', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    ...
    it('should return summary 1 for admin user', async () => {
      await expect(
        adminSession.resources.screenings.screening(adminScreening.id).summary1.get(),
      ).resolves.toMatchObject(emptySummary);
    });

    it('should return summary 1 for regular user', async () => {
      await expect(userSession.resources.screenings.screening(userScreening.id).summary1.get()).resolves.toMatchObject(
        emptySummary,
      );
    });
  });
  ...
  const negativeTestFn = ({ description, sessionType, screeningId, spoofedJwtSubFn, code }) => {
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings.screening(screeningId || adminScreening.id).summary1.get(),

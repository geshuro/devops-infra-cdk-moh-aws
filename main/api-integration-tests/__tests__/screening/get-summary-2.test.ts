...
describe('GET /api/screening/:screeningId/summary2 - Get screening summary 2', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    ...
    it('should return summary 2 for admin user', async () => {
      await expect(
        adminSession.resources.screenings.screening(adminScreening.id).summary2.get(),
      ).resolves.toMatchObject(emptySummary);
    });

    it('should return summary 2 for regular user', async () => {
      await expect(userSession.resources.screenings.screening(userScreening.id).summary2.get()).resolves.toMatchObject(
        emptySummary,
      );
    });
  });
  ...
  const negativeTestFn = ({ description, sessionType, screeningId, spoofedJwtSubFn, code }) => {
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings.screening(screeningId || adminScreening.id).summary2.get(),

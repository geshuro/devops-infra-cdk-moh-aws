...
describe('GET /api/screening/:screeningId/uploadUrl - Get screening upload URL', () => {
  let setup;
  let adminSession;
  let userSession;
  let adminScreening;
  ...
  describe('Positive Tests', () => {
    it('should return an upload URL for admin user', async () => {
      const uploadUrl = await adminSession.resources.screenings.screening(adminScreening.id).uploadUrl.get();
      expect(uploadUrl.s3UploadUrl).not.toHaveLength(0);
    });

    it('should return an upload URL for regular user', async () => {
      const uploadUrl = await userSession.resources.screenings.screening(userScreening.id).uploadUrl.get();
      expect(uploadUrl.s3UploadUrl).not.toHaveLength(0);
    });
  });
  ...
  const negativeTestFn = ({ description, sessionType, screeningId, spoofedJwtSubFn, code }) => {
    it(`rejects "${description}" request with "${code}"`, async () => {
      ...
      await expect(
        session.resources.screenings.screening(screeningId || adminScreening.id).uploadUrl.get(),
